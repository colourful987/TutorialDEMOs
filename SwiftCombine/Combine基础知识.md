...
`map`、`filter`、`reduce` 高阶函数的操作符（个人认为就是面向特定功能的接口），比如map将数据从A->B转换，filter是过滤，reduce是数据合并。当然具体的数据转换、过滤、合并等操作还需要我们来传入闭包，将其作为参数传入 map filter reduce 函数中供其使用，说白了就是封装了一层的高阶函数，但又提供某些自定义操作的空间——这里是传入closure。

如果有一个publisher，不断的map ,filter 操作，我觉得就是将这些操作封装在了publisher实例中，上游数据源传递给订阅者之前，需要经过这一系列操作，才将“二次加工过的数据”传递给订阅者。
